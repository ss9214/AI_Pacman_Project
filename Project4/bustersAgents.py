...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # Get most likely ghost positions
        ghostPositions = list()

        for ghostDist in livingGhostPositionDistributions:
            ghostPositions.append(max(ghostDist, key=ghostDist.get))

        # Find closest ghost
        closestGhost = (pacmanPosition, float("inf"))

        for ghostPos in ghostPositions:
            distance = self.distancer.getDistance(pacmanPosition, ghostPos)

            if distance < closestGhost[1]:
                closestGhost = (ghostPos, distance)

        # Find best legal action (smallest distance to closest ghost)
        bestAction = (Directions.STOP, float("inf"))

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            successorDistance = self.distancer.getDistance(successorPosition, closestGhost[0])

            if successorDistance < bestAction[1]:
                bestAction = (action, successorDistance)
